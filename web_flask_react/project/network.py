...
print 'PAGERANK'
for val, key in sorted([(val, key) for key, val in pagerank_dict.items()], reverse=True)[:10]:
	print labels_dict[key], ':', val

hubs, authorities = nx.hits(G)
print 'HUBS'
for val, key in sorted([(val, key) for key, val in hubs.items()], reverse=True)[:10]:
	print labels_dict[key], ':', val

print 'AUTHORITIES'
for val, key in sorted([(val, key) for key, val in authorities.items()], reverse=True)[:10]:
	print labels_dict[key], ':', val
...
for val, key in sorted([(val, key) for key, val in nx.degree(G).items()], reverse=True)[:10]:
	print labels_dict[key], ':', val

for size, clique_list in sorted([(len(clique_list), clique_list) for clique_list in nx.find_cliques(G)], reverse=True)[:10]:
...
subgraphs = [g for size, g in sorted([(g.number_of_nodes(), g) for g in nx.connected_component_subgraphs(G)], reverse=True)]
for i, g in enumerate(subgraphs[:10]):
	...
	nx.draw_networkx(g, linewidths=0, node_color='#33ccff', edge_color='#cccccc', labels=cur_labels_dict)
	plt.show()
