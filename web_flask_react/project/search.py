...
class Searcher(object):
    def __init__(self, vectorizer, tfidf_mat, word_dict, course_doc_dict,
    ...
    def search(self, term_string, major, certificate, num_results=20):
        ...
        if last_index > 0:
            courses = [
                self.course_id_list[doc_num]
                for doc_num in sorted_docs[:last_index]
            ]
        else:
            courses = []
        # import pdb; pdb.set_trace()
        ...
        if len(split_terms) == 2 and split_terms[0].isalpha() and len(
                split_terms[0]) == 3 and split_terms[1].isdigit() and len(
                    split_terms[1]) == 3:
            specific_courses = self.course_id_lookup_dict[term_string.upper()
                                                          .replace(' ', '')]
        ...
        for cur_id in courses:
            if len(recommendations) >= num_results:
                break
            if not any(
                    map(lambda x: 'FRS' in x, self.class_number_lookup_dict[
                        cur_id])):
