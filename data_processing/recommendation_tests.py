...
doc_list = open(sys.argv[1]).read().replace('"', '').replace(',',
                                                             ' ').split('\n')
"""
...
for line in f:
    ...
    for i in range(0, len(classes), 2):
        ...
        cleaned_class_dict[classes[i] + classes[i + 1]] = line.replace(
            '"', '').replace(',', ' ')
"""
...
vectorizer = TfidfVectorizer(
    input='content', max_df=0.5, stop_words='english', use_idf=True, min_df=2)
...
lda_all_classes = lda.transform(
    vectorizer.transform(cleaned_class_dict.values()))
class_ratings = []
for lda_class in lda_all_classes:
    class_ratings.append(np.dot(lda_class, cluster_scores))

class_rankings = sorted(
    list(zip(class_ratings, cleaned_class_dict.keys())), reverse=True)
