...
TERM_CODES = [1122, 1124, 1134, 1142, 1144, 1152, 1154, 1162, 1164, 1172, 1174]
TERM_CODES_NAMES = [
    'Fall_11', 'Spring_12', 'Spring_13', 'Fall_13', 'Spring_14', 'Fall_14',
    'Spring_15', 'Fall_15', 'Spring_16', 'Fall_16', 'Spring_17'
]
TERM_CODE_DICT = {
    code: name
    for code, name in zip(TERM_CODES, TERM_CODES_NAMES)
}
...
def get_course_details(soup):
    ...
    match = re.match(
        r'\(([A-Z]+)\)', clean(soup('strong')[1].findNext(text=True)))
    ...
    ) if pretitle != None else ''  # broken spring 16 for many courses
    return {
        'courseid': course_id,
        'area':
        area,  #bwk: this was wrong[1:-1],    # trim parens #  match.group(1) if match != None else ''
        'title': title,  # was [1]
...
def get_course_listings(soup):
    ...
    } for match in LISTING_REGEX.finditer(listings)]
...
def get_course_profs(soup):
    ...
    return [{
        'uid': PROF_URL_REGEX.search(link['href']).group('id'),
        'name': clean(link.string)
...
def get_course_classes(soup):
    ...
    return [
        get_single_class(row) for row in class_rows
